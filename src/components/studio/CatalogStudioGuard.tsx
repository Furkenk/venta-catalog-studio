...
export function CatalogStudioGuard(props: Props) {
  ...
  const commit = React.useCallback((next: Page[]) => {
    setLastPages(pages.map(p => ({ ...p, content: { ...p.content, blocks: [...(p.content.blocks || [])] } })));
    setPages(next);
  ...
  React.useEffect(() => {
    const onProductClick = (event: MouseEvent) => {
      ...
      requestAnimationFrame(() => {
        ...
        const elementsButton = buttons.find(b => b.textContent?.trim() === 'Eleman');
  ...
  return <div className="relative w-full h-full">
    ...
    <button
      onClick={undo}
      disabled={!lastPages}
      className="fixed left-[190px] bottom-5 z-[85] h-10 px-4 bg-white border border-[#0f203a]/15 shadow-lg text-[9px] uppercase tracking-[.12em] disabled:opacity-30"
    >Geri Al</button>
    <ProductSelectionPanelV2
