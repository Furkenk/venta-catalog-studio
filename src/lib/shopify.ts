...
export function connectShopify(shop:string){
  ...
  // The app is embedded inside Shopify Admin. OAuth must leave the iframe;
  // otherwise accounts.shopify.com can refuse to render inside the frame.
...
export async function fetchShopifyProducts():Promise<{products:Product[];shop:string;shopName?:string}>{
  ...
  if(!r.ok)throw new Error(d.error||'Shopify ürünleri alınamadı.');
  return d;
}
...
export async function loadCatalogState():Promise<{catalog:Catalog;pages:Page[]}|null>{
  ...
  return d.catalog.catalog&&d.catalog.pages?d.catalog:null;
}

export async function saveCatalogState(catalog:Catalog,pages:Page[]){
  const r=await fetch('/api/catalog',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({catalog:{catalog,pages}})});
  const d=await r.json();
  if(!r.ok)throw new Error(d.error||'Katalog kaydedilemedi.');
  return d;
}
