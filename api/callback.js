...
module.exports = async (req, res) => {
  try {
    ...
    if (!/^[a-z0-9][a-z0-9-]*\.myshopify\.com$/.test(shopName)) {
      return res.status(400).send('Geçersiz mağaza.');
    }

    if (!verifyShopifyHmac(req.query)) {
      return res.status(403).send('Geçersiz Shopify HMAC.');
    }
    ...
    if (
      !savedState ||
      !savedSig ||
      !timingSafeEqualString(savedState, String(state)) ||
      !timingSafeEqualString(savedSig, sign(savedState))
    ) {
    ...
    try {
      ...
      console.error('Shopify token endpoint returned non-JSON:', response.status, responseText.slice(0, 500));
      return res.status(502).send(`Shopify token exchange başarısız (${response.status}).`);
    }

    if (!response.ok || !data.access_token) {
      console.error('Shopify token exchange failed:', response.status, data);
      return res.status(502).send('Shopify yetkilendirme başarısız.');
    }
    ...
    const session = encryptSession({
      ...
      scope: data.scope || process.env.SHOPIFY_SCOPES || 'read_products',
